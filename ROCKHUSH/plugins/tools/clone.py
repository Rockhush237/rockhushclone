...
from ROCKHUSH.utils.database import get_assistant, clonebotdb
from ROCKHUSH.utils.database.clonedb import has_user_cloned_any_bot
from config import LOGGER_ID, CLONE_LOGGER
import requests
from ROCKHUSH.utils.decorators.language import language
import pyrogram.errors

from ROCKHUSH.utils.database.clonedb import get_owner_id_from_db
    ...
    if len(message.command) > 1:
        ...
        try:
            ai = Client(
                ...
                API_HASH,
                bot_token=bot_token,
                plugins=dict(root="ROCKHUSH.cplugin"), 
            )
            await ai.start()
            bot = await ai.get_me()
        ...
        try:
            ...
            await mi.edit_text(
                f"⚠️ <b>ᴇʀʀᴏʀ:</b>\n\n<code>{e}</code>\n\n**ᴋɪɴᴅʟʏ ғᴏᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ [sᴜᴘᴘᴏʀᴛ]({SUPPORT_CHAT}) ᴛᴏ ɢᴇᴛ ᴀssɪsᴛᴀɴᴄᴇ**"
            )
    else:
