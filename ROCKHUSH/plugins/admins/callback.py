...
from ROCKHUSH import YouTube, app
from ROCKHUSH.core.call import INNOCENT
from ROCKHUSH.misc import SUDOERS, db
...
from ROCKHUSH.utils.formatters import seconds_to_min
from ROCKHUSH.utils.inline import close_markup, stream_markup, stream_markup_timer
from ROCKHUSH.utils.stream.autoclear import auto_clean
from ROCKHUSH.utils.thumbnails import get_thumb
import config
from config import (
    BANNED_USERS,
    SOUNCLOUD_IMG_URL,
...
async def unban_assistant(_, callback: CallbackQuery):
    ...
    try:
        ...
        await callback.answer("**ᴀssɪsᴛᴀɴᴛ Jɪ ᴜɴʙᴀɴ ʜᴏ ɢᴀᴇᴇᴇ ʜᴇʜᴇ**🥳\n\n► **ᴀʙ ᴍᴀJᴇ ᴋʀᴏ ɢᴀɴɴᴀ ᴄʜᴀʟᴀ ᴋʀ **👀✨\n\n**ᴀᴀᴘᴋᴀ ᴀᴀʙʜᴀʀ**💝", show_alert=True)
    except Exception as e:
        await callback.answer(f"**ʙᴀɴ ᴘᴏᴡᴇʀ ɴᴏɪ ᴍᴇʀᴇ ᴘᴀss** 😒 **ᴋᴇsᴇ ᴋᴀʀᴜ ᴜɴʙᴀɴ ᴀssɪsᴛᴀɴᴛ Jɪ ᴋᴏ**🥺\n\n➻ **ʙᴀɴ ᴘᴏᴡᴇʀ ᴅᴇᴅᴏ ɴᴀ** 🥺🥺", show_alert=True)
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        if command == "Skip":
            txt = f"► **Yᴇ ʜᴀɪ ɢᴀɴᴀ ᴄʜᴀɴɢᴇ ᴋʀɴᴇ ᴡᴀʟᴀ**😒\n│ \n└»» : {mention} 🥀"
            popped = None
            try:
                popped = check.pop(0)
                if popped:
                ...
                if not check:
                    await CallbackQuery.edit_message_text(
                        f"► **Yᴇ ʜᴀɪ ɢᴀɴᴀ ᴄʜᴀɴɢᴇ ᴋʀɴᴇ ᴡᴀʟᴀ**😒\n│ \n└»» : {mention} 🥀"
                    )
                    await CallbackQuery.message.reply_text(
