...
from ROCKHUSH.utils.ROCKHUSH_ban import admin_filter
...
async def pin(_, message):
    ...
    if message.chat.type == enums.ChatType.PRIVATE:
        await message.reply_text("**ᴀʀᴇ ᴘᴀɢʟᴜ sɪʀғ ɢʀᴏᴜᴘ ᴍᴇ ᴜsᴇ ʜᴏᴛᴀ ʜᴀɪ ʏᴇ** 🤭🤭")
    elif not replied:
        await message.reply_text("**ᴍᴇssᴀɢᴇ ᴘᴀʀ ʀᴇᴘʟʏ ᴋᴀʀᴋᴇ ʙᴀᴛᴀᴏ ᴋᴏɴsᴀ ᴋᴀʀɴᴀ ʜᴀɪ ᴘɪɴ 😩**")
    else:
        user_stats = await app.get_chat_member(chat_id, user_id)
        if user_stats.privileges.can_pin_messages and message.reply_to_message:
            try:
                await message.reply_to_message.pin()
                await message.reply_text(f"**ᴘɪɴ ᴋᴀʀ sɪʏᴀ sɪʀ Jɪ 🫡**\n\n**ᴄʜᴀᴛ:** {chat_title}\n\n**ᴀᴅᴍɪɴ:** {name}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(" 📝 ᴠɪᴇᴡs ᴍᴇssᴀɢᴇ ", url=replied.link)]]))
...
async def pinned(_, message):
    ...
    if not chat.pinned_message:
        return await message.reply_text("**ᴋᴏɪ ᴘɪɴ ᴡᴀʟᴀ ᴍᴇssᴀɢᴇ ʜɪ ɴᴀʜɪ ʜᴀɪ 😕😕**")
    try:        
        await message.reply_text("ʏᴇ ʀʜᴀ ᴀʙʜɪ ᴘɪɴ ᴋɪʏᴀ ʜᴜᴀ ᴍᴇssᴀɢᴇ 😗😗",reply_markup=
...
async def unpin(_, message):
    ...
    if message.chat.type == enums.ChatType.PRIVATE:
        ...
        await message.reply_text("**ʀᴇᴘʟʏ ᴋᴀʀᴋᴇ ʙᴀᴛᴀᴏ ɴᴀ ᴋᴏɴsᴀ ᴜɴᴘɪɴ ᴋᴀʀɴᴀ ʜᴀɪ 😩**")
    else:
        user_stats = await app.get_chat_member(chat_id, user_id)
        if user_stats.privileges.can_pin_messages and message.reply_to_message:
            try:
                await message.reply_to_message.unpin()
                await message.reply_text(f"**ᴜɴᴘɪɴ ᴋᴀʀ ᴅɪʏᴀ sɪʀ Jɪ 🫡**\n\n**ᴄʜᴀᴛ:** {chat_title}\n**ᴀᴅᴍɪɴ:** {name}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(" 📝 ᴠɪᴇᴡs ᴍᴇssᴀɢᴇ ", url=replied.link)]]))
            except Exception as e:
                await message.reply_text(str(e))
...
async def deletechatphoto(_, message):
      ...
      chat_id = message.chat.id
      user_id = message.from_user.id
      msg = await message.reply_text("**ᴋʀᴛᴀ ʜᴜ ʀᴜᴋᴏ 😁....**")
      admin_check = await app.get_chat_member(chat_id, user_id)
      if message.chat.type == enums.ChatType.PRIVATE:
           await msg.edit("**ᴀʀᴇ ᴘᴀɢʟᴜ sɪʀғ ɢʀᴏᴜᴘ ᴍᴇ ᴜsᴇ ʜᴏᴛᴀ ʜᴀɪ ʏᴇ** 🤭🤭") 
      try:
